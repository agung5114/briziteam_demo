...
def search(request):
    ...
    df = df.drop('hasil_putusan', axis=1)
    df = df.merge(dfnpwp[['file_names','hasil_putusan']],how='left',on='file_names')
    plist = []
    for ps in df['pasal']:
        for p in ps:
            plist.append(p)
    ...
    context = {'form': form,'total':len(dfall.index),'chart1':fig1.to_html(),'chart2':fig2.to_html(),'selected':topic,
            #    'table2':dfff,
                'files':df,'first':first,'topik':topic
                }
...
def pdf_extract(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            dftext = dftext.drop('hasil_putusan', axis=1)
            dftext = dftext.merge(dfnpwp[['file_names','hasil_putusan']],how='left',on='file_names')
            ...
            return render(request, 'pdf_extract.html', {'form': form,'text':text[:3150],'topic':topic[1],'linkpdf':linkpdf,'figtopik':figtopik.to_html(),
                                                        'similarity':similarity,'texthasil':texthasil[:2150],'figpasal':figpasal.to_html(),'npwp':npwp,'first':first,
