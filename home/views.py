...
def search(request):
    ...
    fig3 = px.bar(dftahun.sort_values(by='tahun', ascending=True), x='tahun', y='file_names', 
    ...
    fig2.update_traces(textinfo='label+value+percent',insidetextorientation='horizontal')

    dftext = get_sengketa_multi(df['file_names'].tolist(),topic.lower())
    # try:
    #     df = df[df['file_names'].isin(check_file())]
    # except:
    #     pass
    ...
    df = df.sort_values('jumlah',ascending=False)[:int(jumlah)]
    df['rank'] = df['jumlah'].rank(method='first',ascending=False)
    ...
    # dfirst = df.loc[:1]
    if df.empty:
        first = '001097.16.2018w'
    else:
        first = df['file_names'].str.replace('.txt','').values[0]
    # first = df['file_names'].iloc[0]
    # first = first.replace('.txt','')

    context = {'form': form,'total':len(dfall.index),'chart1':fig1.to_html(),'chart2':fig2.to_html(),'chart3':fig3.to_html(),
               'selected':topic,
            #    'table2':dfff,
                'topik':topic,
...
def open_pdf(request,id,key):
    ...
    doc.save(os.path.join(BASE_DIR,os.path.join('media','highlighted.pdf')), garbage=4, deflate=True, clean=True)
    link = os.path.join(BASE_DIR,os.path.join('media','highlighted.pdf'))
...
def pdf_extract(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            doc = fitz.open(stream=my_file.read())
            ### OUTPUT
            doc.save(os.path.join(BASE_DIR,'media/extracted_page.pdf'), garbage=4, deflate=True, clean=True)
            ...
            similarity = get_sim(features,topic[0])
            if len(similarity[similarity['similarity']>=0.95])>1:
                similarity = similarity[similarity['similarity']>=0.95]
            ...
            return render(request, 'pdf_extract.html', {'form': form,'topic':topic[1],'linkpdf':linkpdf,'figtopik':figtopik.to_html(),
                                                        'similarity':similarity,'figpasal':figpasal.to_html(),'npwp':npwp,'first':first,
                                                        # 'text':text[:100],'texthasil':texthasil[:100],
...
def detail_putusan(request, id):
    ...
    figpasal.update_layout(coloraxis_showscale=False,xaxis_title=None,yaxis_title=None,height=300)
    dftopik = pd.read_feather(os.path.join('media', 'countTopik.ft'))
    toptopik = dftopik[dftopik['file_names'].isin([df['file_names'].values[0]])]
    figtopik = px.bar(toptopik.sort_values('jumlah_kata'),x='jumlah_kata',y='kata_kunci',
                      orientation='h',color='jumlah_kata',color_continuous_scale=["#ffedcf","#ec8569","#de425b"])
    figtopik.update_layout(coloraxis_showscale=False,xaxis_title=None,yaxis_title=None,height=300)
    ...
    noput = dfsum[dfsum['file_names'].isin([similarity['file_names'].values[0]])]['noput'].values[0]
    return render(request, 'detail_putusan.html', {'topic':topic[1],'file':file,'noput' :noput,'figpasal':figpasal.to_html(), 'similarity':similarity,
                                                # 'textputusan':textputusan[:2150],'text':text[:3150],
                                                'linkpdf':linkpdf,'figtopik':figtopik.to_html(), 
...
def audittrail(request):
    connection2 = sqlite3.connect(os.path.join(BASE_DIR, 'db.sqlite'),check_same_thread=False)
    query1 = f'''select * from SEARCHLOG
                '''
    query2 = f'''select * from ACTLOG
                '''
    df1 = pd.read_sql_query(query1,connection2)
    df2 = pd.read_sql_query(query2,connection2)

    return render(request,'loguser.html',{'searchdata':df1,'logdata':df2})
