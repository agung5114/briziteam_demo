...
dfstop = pd.read_csv(os.path.join('', 'media/stop.csv'))


def get_sengketa(texts):
    query1 = None
    if len(texts)==1:
        query1 = f'''select file_names,hasil_putusan from WORDS
                    where instr(words, '{texts[0]}') > 0
                    '''
    elif len(texts)==2:
        query1 = f'''select file_names,hasil_putusan from WORDS
                    where instr(words, '{texts[0]}') > 0
                    and instr(words, '{texts[1]}') > 0
                    '''
    elif len(texts)>=3:
        query1 = f'''select file_names,hasil_putusan from WORDS
                    where instr(words, '{texts[0]}') > 0
                    and instr(words, '{texts[1]}') > 0
                    and instr(words, '{texts[2]}') > 0
                    '''

    df = pd.read_sql_query(query1,connection)
    return df

def get_sengketaFilter(texts,hasil):
    query1 = None
    if len(texts)==1:
        query1 = f'''select file_names,hasil_putusan from WORDS
                    where instr(words, '{texts[0]}') > 0
                    and hasil_putusan in ({"'"+str(hasil)+"'"})
                    '''
    elif len(texts)==2:
        query1 = f'''select file_names,hasil_putusan from WORDS
                    where instr(words, '{texts[0]}') > 0
...
def get_sengketaText(filelist):
    query1 = f'''select file_names, words as kalimat,counts from WORDS
...
#                 where file_names in ({','.join(["'"+str(x)+"'" for x in filelist])})
#                 '''
#     df = pd.read_sql_query(query1,connection2)
#     # pokok = df['pokok_sengketa'].tolist()
#     return df

def get_sengketa_multi(filelist,text):
    query1 = f'''select (LENGTH(pokok_sengketa) - LENGTH(REPLACE(pokok_sengketa, '{text}', ''))) as jumlah,file_names from SENGKETA
                where file_names in ({','.join(["'"+str(x)+"'" for x in filelist])})
                '''
    df = pd.read_sql_query(query1,connection)
    # pokok = df['pokok_sengketa'].tolist()
...
#     query1 = f'''select substr(putusan,1,3500) as putusan from SENGKETA
#                 where file_names in ('{filename}')
#                 '''
#     df = pd.read_sql_query(query1,connection)
#     return df
